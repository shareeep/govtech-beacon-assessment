...
description: A multi-agent anonymisation system using Mistral 7B, BERT NER, and adversarial optimisation that achieves 97% anonymisation rate while retaining 92% semantic utility on clinical documents.
...
projectType: "AI/ML"
highlights:
  - Multi-agent pipeline achieving 97% anonymisation rate with 92% semantic utility (BERTScore)
  - Fine-tuned BERT NER (Micro-F1 0.87) with Automatic Prompt Engineering via evolutionary mutation
  - Semantic segmentation cutting PII leakage by 42% on long clinical documents
  - Full-stack app with FastAPI + LangChain backend and Next.js frontend with DOCX round-trip support
---
