...
description: "AWS big data pipeline (S3, Glue, EMR, Athena) processing 120M+ flight records with PySpark to identify aircraft that act as network delay multipliers, with a two-stage cascade prediction model."
...
projectType: "DATA"
highlights:
  - End-to-end AWS pipeline (S3, Glue, EMR, Athena) processing 120M+ flight records with PySpark
  - Two-stage cascade prediction model (GBT + MLP) handling 95/5 class imbalance via Spark MLlib
  - Tail-level risk scoring revealing 0.4% of aircraft cause disproportionate network-wide delays
---
